  ...
  return (
    <>
      <div className="min-h-screen w-full bg-gradient-to-r from-g1 to-g2 text-white">
        ...
        <h1 className="mt-8 text-center text-[30px]">Odpowiedź:{getmianownik > 0 ? <span className="ml-4 text-green-400">{getlicznik == 0 ? alert("Nie można przedstawić w postaci ułamka egipskiego") : getlicznik > getmianownik ? null : answer}</span> : alert("Nie dziel przez zero!")}</h1> 
        <UlamekFareya />
      </div>
    </>
  )
